...
# ==========================================
# 1. 페이지 설정 및 모바일 UX 최적화 (사이드바 감춤 처리)
# ==========================================
...
with st.expander("👤 내 계정 및 시스템 설정 (모바일은 여기를 눌러주세요!)", expanded=not st.session_state.logged_in):
    ...
    with set_col:
        ...
        if tele_token and tele_chat_id: st.success("✅ 텔레그램 봇 연동 완료")
        else:
            tele_token = st.text_input("Telegram Bot Token", type="password").strip()
...
with tab1:
    ...
    if df is not None and not df.empty:
        st.markdown("---")
        st.markdown("### 🤖 Harness 4-Agent AI 분석 엔진 (개인화 + 매크로 추가)")
            ...
            with st.spinner("4명의 AI 에이전트가 토론 중입니다... (약 10~20초 소요)"):
                ...
                {{
                  ...
                  "technicalAgent": {{
                    ...
                    "reasoning": "기술적 분석가 에이전트의 차트 및 추세 기반 심층 분석 의견 (2~3문장)"
                  }},
                  "fundamentalAgent": {{
                  ...
                  "riskManager": {{
                    ...
                    "reasoning": "앞선 세 에이전트의 의견과 사용자의 '투자 성향'을 종합하여 리스크 관리자가 내리는 최종 결론 (3~4문장)"
                  }}
                }}
                """
                try:
                    ...
                    st.error(res['riskManager']['reasoning']) # 시각적 분리를 위해 error(빨간 배경) 사용
            ...
            if st.button("🌅 오늘의 모닝 브리핑 생성", type="primary", use_container_width=True):
                with st.spinner("거시경제 및 시장 동향 분석 중..."):
                    try:
                        ...
                        * 핵심 수칙: 매도 등 대응 타이밍은 절대적으로 '지표(차트)'에 의존하고, '재무'와 '뉴스'는 비중 조절용으로 활용하세요.
