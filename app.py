...
def calculate_cloud_indicators(df):
    ...
    try:
        ...
        if ha_body > 0 and ha_range > 0: 
            lower_shadow = latest['HA_Open'] - latest['HA_Low']
            if lower_shadow <= (ha_range * 0.05): 
                if ha_body / ha_range >= 0.5: 
...
with tab1:
    ...
    chart_days = st.slider("🔍 차트 표시 캔들 수 (스마트폰에서는 40~60개가 보기 좋습니다)", min_value=30, max_value=200, value=90, step=10, help="화면 너비에 맞춰 차트에 표시할 캔들의 개수를 조절하세요. 두 손가락으로 줌인/줌아웃도 가능합니다.")
    ...
    if df is not None and not df.empty:
        # 💡 [모바일 최적화 적용] 슬라이더에서 설정한 값만큼의 데이터만 차트에 렌더링
        ...
        # 💡 [Y축 동적 스케일링 엔진] 스마트폰 줌인 효과 완벽 보장
        # 멀리 있는 200일선 때문에 캔들이 위아래로 납작하게 찌그러지는 현상을 강제로 차단하고, 화면에 꽉 차게 확대
        y_cols_max = ['HA_High', 'BB_Upper'] if show_heikin_ashi else ['High', 'BB_Upper']
        y_cols_min = ['HA_Low', 'BB_Lower'] if show_heikin_ashi else ['Low', 'BB_Lower']
        y_max = float(display_df[y_cols_max].max().max())
        y_min = float(display_df[y_cols_min].min().min())
        y_padding = (y_max - y_min) * 0.15 # 상하 15% 여유 공간 확보
        
        fig.update_yaxes(
            range=[y_min - y_padding, y_max + y_padding],
            tickformat="$,.2f" if is_us_stock else ",.0f", 
            ticksuffix="" if is_us_stock else "원", 
            row=1, col=1
        )
                ...
                try:
                    ...
                    if ind:
                        ...
                        if sc >= 2 and ind.get("Is_Above_Monthly_EMA10"):
                            ...
                            tags = []
                            if ind.get('HA_ScrewBar'): tags.append("🍦HA스크류바")
