...
# 💡 복구 완료: AI 호출 무적의 방탄 코드 적용 (잘림 현상 완벽 수정본)
...
def get_ai_analysis(prompt, api_key):
    ...
    for attempt in range(5):
        try:
            ...
            # 1. 429 할당량 초과 에러 시 구글이 요구하는 대기시간을 읽고 알아서 기다림
            if "429" in error_str or "quota" in error_str:
                if attempt < 4:
                    match = re.search(r'retry in (\d+\.?\d*)s', error_str)
                    if match:
                        wait_time = float(match.group(1)) + 1.0 # 요구시간보다 1초 더 안전하게 대기
                    else:
                        wait_time = 15.0 # 요구시간이 안 적혀있으면 15초 대기
                    time.sleep(wait_time)
                    continue
            
            # 2. JSON 파싱 에러(모델이 이상한 대답을 한 경우) 가벼운 재시도
            elif "json" in error_str or "expecting value" in error_str:
                if attempt < 4:
                    time.sleep(2)
                    continue
            
            # 5번 다 실패하면 밖으로 에러 던지기
            raise e
...
with tab1:
    ...
    if df is not None and not df.empty:
        if st.button("🚀 클라우드 3-Agent 분석 실행", type="primary", use_container_width=True):
            with st.spinner("클라우드 단기 타점과 월봉 10선 장기 추세를 종합하여 계산 중입니다... (API 한도 도달 시 약 30~40초 자동 대기할 수 있습니다)"):
                ...
                try:
                    ...
                    if "429" in error_msg or "quota" in error_msg:
                        st.error("⏳ 구글 AI 일일 무료 호출 한도를 초과했습니다. 너무 잦은 분석으로 인해 오늘 한도가 소진되었을 수 있습니다. 내일 다시 시도해주세요!")
                    else:
                        st.error(f"오류 발생: {e}\n(응답이 일시적으로 지연되었습니다. 다시 버튼을 눌러주세요.)")
...
with tab2:
    ...
    if not st.session_state.portfolio.empty:
        ...
        if st.button("✨ 포트폴리오 전체 진단받기", type="primary", use_container_width=True):
            with st.spinner("클라우드 기법(터틀 손절선, 200일선 등)을 적용하여 전략을 계산 중입니다... (API 한도 도달 시 대기시간 발생)"):
                pf_text = ""
                for idx, row in display_df.iterrows():
                    actual_n, tck = get_stock_info(row['종목명'])
                ...
                prompt = f"""
                당신은 '클라우드 주식 기법'과 '월봉 10선 기법'을 마스터한 냉철한 퀀트 투자 리스크 관리자입니다.
                ...
                try:
                    ...
                    error_msg = str(e).lower()
                    if "429" in error_msg or "quota" in error_msg:
                        st.error("⏳ 구글 AI 일일 무료 호출 한도를 초과했습니다. 내일 다시 시도해주세요!")
                    else:
                        st.error(f"오류 발생: {e}")
